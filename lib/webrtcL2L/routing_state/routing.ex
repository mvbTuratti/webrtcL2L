defmodule WebrtcL2L.RoutingState.Routing do
  ...
  @impl true
  def handle_call({:create_high_quality_stream, streamer}, _, current_state) do
    current_state = Kernel.put_in(current_state, [:high_quality, streamer], Graph.new())
    {:reply, :ok, current_state, @timeout}
  end
  @impl true
  def handle_call({:create_low_quality_stream, streamer}, _, current_state) do
    current_state = Kernel.put_in(current_state, [:low_quality, streamer], Graph.new())
    {:reply, :ok, current_state, @timeout}
  end
  @impl true
  def handle_call({:create_audio_only_stream, streamer}, _, current_state) do
    current_state = Kernel.put_in(current_state, [:audio_only, streamer], Graph.new())
    {:reply, :ok, current_state, @timeout}
  end
  def handle_call({:add_viewer_high_quality, {streamer, viewer}}, _, current_state) do
  ...
  def create_stream(pid, type, streamer) do
    GenServer.call(pid, {type, streamer})
  end
  # @impl true
