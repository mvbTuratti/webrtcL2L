defmodule ConferenceWeb.RecommendationTest do
  alias Conference.Routing.Recommendation
  ...
  describe "join_viewer/4" do
    ...
    test "should use root as streamer", state do
      ...
      {:ok, _graph, streamer} = Recommendation.join_viewer(state[:graph],"joao","matheus", weights)
      assert streamer == "joao"
    end
    test "should use flavio as streamer", state do
      {:ok, _graph, streamer} = Recommendation.join_viewer(state[:graph], "joao", "matheus", [{"flavio", 1}])
    ...
    test "should only have vertices to streamer", state do
      weights = [{"joao", 6}, {"maria", 4}, {"flavio", 3}, {"zeze", 1}]
      {:ok, graph, _streamer} = Recommendation.join_viewer(state[:graph], "joao", "matheus", weights)
    ...
    test "returns error if watcher only connects to peers not watching the stream", state do
      {atom, _graph, _streamer} = Recommendation.join_viewer(state[:graph], "joao", "matheus", [{"cunha", 1}])
      assert atom == :missing_streamer
    end
  end
  ...
  describe "remove_viewer/2" do
    ...
    test "removing a streamer should return the names of affected viewers", state do
      {_graph, affected_viewers} = Recommendation.remove_viewer(state[:graph], "maria")
