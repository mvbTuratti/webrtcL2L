defmodule SdpTable.PerfectNegotiation.PerfectNegotiationTest do
  ...
  alias Conference.SdpTable.MediaStructs.ParticipantMedia
  ...
  describe "upsert_screen_sharing_value/4" do
    setup do
      negotiation_values = create_two_participants()
      state = for participant <- negotiation_values, reduce: %{} do
        acc -> PerfectNegotiation.upsert_routing_values_to_member(acc, participant, "user", "zeze#{participant.high_quality}")
  ...
  describe "get_screen_sharing_sdp_value/3" do
    setup do
      negotiation_values = create_two_participants()
      state = for participant <- negotiation_values, reduce: %{} do
        acc -> PerfectNegotiation.upsert_routing_values_to_member(acc, participant, "user", "zeze#{participant.screen_sharing}")
      end
      {:ok, %{sdp_state: state, participants: negotiation_values}}
    end
    test "retrieve successfully value from existing member", state do
      assert {:ok, "2"} = PerfectNegotiation.get_screen_sharing_sdp_value(state.sdp_state, "user", "zeze2")
    end
    test "return error message when routee does not exists", state do
      assert {:missing_value, ""} = PerfectNegotiation.get_screen_sharing_sdp_value(state.sdp_state, "user", "zeze3")
    end
    test "return error message when user does not exists", state do
      assert {:missing_value, ""} = PerfectNegotiation.get_screen_sharing_sdp_value(state.sdp_state, "user1", "zeze3")
    end
  end

  def create_two_participants() do
    Enum.map(1..2, fn iteration ->
      %ParticipantMedia{high_quality: "#{iteration}", low_quality: "#{iteration}", audio_only: "#{iteration}", screen_sharing: "#{iteration}"}
    end)
  end
end
